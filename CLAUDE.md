...
```bash
uv sync --extra analytics  # install deps (include analytics/ETL extras)
uv run python -m src.main  # run locally
```

## Deployment

When updating a running install (`git pull` + restart), follow the ritual in [ARCHITECTURE.md → Operations → Updating a Running Install](ARCHITECTURE.md#updating-a-running-install). The `git pull` → `systemctl restart` shortcut is **wrong** — service units use `uv run --no-sync`, so you must run `sudo -u tars uv sync --extra analytics` between the pull and the restart or the service will crash on a new dep or silently run stale code.
